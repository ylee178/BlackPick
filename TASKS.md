...
- **Review gate**: primary path is the user-level `second-opinion-reviewer` subagent — invoke via "Use the second-opinion-reviewer subagent to review <artifact>". Sonnet 4.6 default, Opus 4.6 escalation on low-confidence BLOCK. Usage guide: `/Users/uxersean/Desktop/Wiki_Sean/Tech/second_opinion_reviewer_usage.md`. Supplements — does NOT replace — cross-family external review; reach for external when stakes justify the cost. Historical fallback (deprioritized for cost): `scripts/codex-review.sh` → `scripts/gpt-review.sh`, see `Docs/codex-review.md`. Never call the OpenAI API from anywhere else. Docs-only PRs (TASKS.md, wiki, Docs/) exempt — self-review OK.
