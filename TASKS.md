...
_Last updated: 2026-04-13 (PR #27 shipped Branch 7 `feature/onboarding-first-time-flow`, Phase 1 now 8/9; `public/email/bp-logo-email.png` cherry-picked to main as `20ffbd6` — all 3 PROD email assets now 200, Supabase test-email verification unblocked; remaining Phase 1: Branch 8 `feature/streak-ux`, Branch 9 `fix/verify-all-predicted-toast`)_
...
1. **Start on develop**: `git checkout develop && git pull`. Verify `69a53b6 chore(docs): session wrap 2026-04-13 — PR #27 Branch 7 shipped, Phase 1 → 8/9` is the tip or close to it. Main tip is `20ffbd6` (logo PNG cherry-pick).
...
3. **Supabase email templates — final verification step**: ~~Sean opens the preview asset URLs and verifies 200~~ **DONE this session** — all 3 assets now live on PROD: `https://blackpick.io/email/{bp-logo-email.png, icon-shield, icon-key}` → 200 after the `20ffbd6` single-file cherry-pick of the logo PNG to main. Templates already SAVED in Supabase dashboard earlier this session (confirm_signup + reset_password slots). **Remaining manual step (Sean-only)**: Supabase Dashboard → Authentication → Email Templates → **Send Test Email** for at least `confirm_signup` and `reset_password`, verify real Gmail/iOS Mail/Outlook rendering (logo + lucide icons + copy). The dashboard Preview tab shows broken images because Supabase Preview does not substitute `{{ .SiteURL }}` — this is expected, NOT a bug; actual sent emails substitute correctly. Once test emails render cleanly, the PR #25 follow-up is fully closed.
