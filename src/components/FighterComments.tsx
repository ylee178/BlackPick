...
function TranslatableBody({ commentId, body }: { commentId: string; body: string }) {
  ...
  function renderBody(text: string) {
    return text.split(/(@\S+)/g).map((part, i) =>
      part.startsWith("@") ? <span key={i} className="font-semibold text-[var(--bp-accent)]">{part}</span> : <span key={i}>{part}</span>
    );
  }

  return (
    <>
      <span className={cn("text-[var(--bp-ink)] opacity-90", loading && "animate-shimmer bg-gradient-to-r from-[var(--bp-muted)] via-[var(--bp-ink)] to-[var(--bp-muted)] bg-[length:200%_100%] bg-clip-text text-transparent")}>
        {renderBody(displayText)}
      </span>
      {!isSameLanguage && (
        <button type="button" onClick={handleTranslate} disabled={loading} className="ml-1 text-xs font-semibold text-[var(--bp-accent)] hover:opacity-80">
...
function HeartIcon({ filled, className }: { filled: boolean; className?: string }) {
  return <Heart className={cn("h-4 w-4", filled && "text-[var(--bp-danger)]", className)} fill={filled ? "currentColor" : "none"} strokeWidth={2} />;
}
...
function CommentRow({ comment, isReply, currentUserId, onDelete, onLike, onReply, t }: {
  ...
  return (
    <div className="flex gap-2.5">
      <div className={cn("flex shrink-0 items-center justify-center rounded-full bg-[var(--bp-accent-dim)] font-bold text-[var(--bp-accent)]", isReply ? "h-6 w-6 text-[10px]" : "h-8 w-8 text-xs")}>{initial}</div>
      <div className="min-w-0 flex-1">
        <p className={cn("leading-snug", isReply ? "text-[13px]" : "text-sm")}>
          <span className="font-semibold text-[var(--bp-ink)]">{comment.users?.ring_name ?? "?"}</span>{" "}
        ...
        <div className="mt-1 flex items-center gap-3 text-xs text-[var(--bp-muted)]">
          ...
          <button type="button" onClick={() => onLike(comment.id)} disabled={!currentUserId}
            className={cn("flex cursor-pointer items-center gap-1 transition-transform active:scale-125", !currentUserId && "pointer-events-none opacity-30")}>
          ...
          {currentUserId && <button type="button" onClick={() => onReply(comment)} className="cursor-pointer font-semibold text-[var(--bp-muted)] hover:text-[var(--bp-ink)]">{t("discussion.reply")}</button>}
          {currentUserId === comment.user_id && <button type="button" onClick={() => onDelete(comment.id)} className="cursor-pointer font-semibold text-[var(--bp-muted)] hover:text-[var(--bp-danger)]">{t("discussion.delete")}</button>}
        </div>
      </div>
    </div>
  );
...
function CommentThread({ comment, replies, currentUserId, onDelete, onLike, onReplySubmit, replyingTo, onSetReplyTo, replyMention, t }: {
  comment: Comment; replies: Comment[]; currentUserId: string | null;
  onDelete: (id: string) => void; onLike: (id: string) => void; onReplySubmit: (pid: string, body: string) => Promise<void>;
  replyingTo: string | null; onSetReplyTo: (id: string | null, mention?: string) => void; replyMention: string; t: (k: string) => string;
}) {
  ...
  function handleReply(target: Comment) {
    ...
    if (replyingTo === comment.id && replyMention === mention) { onSetReplyTo(null); }
    else { onSetReplyTo(comment.id, mention); if (!expanded && replyCount > 0) setExpanded(true); }
  }

  return (
    <div>
      <CommentRow comment={comment} isReply={false} currentUserId={currentUserId} onDelete={onDelete} onLike={onLike} onReply={handleReply} t={t} />
      {replyCount > 0 && (
        <div className="ml-[19px] mt-2">
          <button type="button" onClick={() => setExpanded(!expanded)} className="flex cursor-pointer items-center gap-1.5 text-xs font-semibold text-[var(--bp-accent)] hover:opacity-80">
            {expanded ? <ChevronUp className="h-3.5 w-3.5" strokeWidth={2.5} /> : <ChevronDown className="h-3.5 w-3.5" strokeWidth={2.5} />}
            {expanded ? t("discussion.hideReplies") : t("discussion.viewReplies").replace("{count}", String(replyCount))}
          </button>
          {expanded && (
            <div className="mt-2 space-y-3 border-l-2 border-[var(--bp-line)] pl-4">
              {replies.map((reply) => <CommentRow key={reply.id} comment={reply} isReply currentUserId={currentUserId} onDelete={onDelete} onLike={onLike} onReply={handleReply} t={t} />)}
            </div>
          )}
        </div>
      )}
...
function CommentSkeleton({ isReply = false }: { isReply?: boolean }) {
  return (
    <div className="flex gap-2.5">
      <div className={cn("shrink-0 animate-shimmer rounded-full bg-gradient-to-r from-[var(--bp-card-inset)] via-[rgba(255,186,60,0.08)] to-[var(--bp-card-inset)] bg-[length:200%_100%]", isReply ? "h-6 w-6" : "h-8 w-8")} />
...
export default function FighterComments({ fighterId, currentUserInitial }: { fighterId: string; currentUserInitial?: string }) {
  const { t } = useI18n();
  ...
  const fetchComments = useCallback(async () => {
    ...
    if (isFirst) { setLoading(true); setShowContent(false); }
    try {
      const res = await fetch(`${API_BASE}?fighter_id=${fighterId}`);
      const data = await res.json();
      if (res.ok) {
        if (isFirst) {
          const elapsed = Date.now() - start;
          await new Promise((r) => setTimeout(r, Math.max(0, MIN_SKELETON_MS - elapsed)));
          isInitialLoad.current = false;
        }
        setComments(data.comments ?? []);
        if (data.current_user_id) setCurrentUserId(data.current_user_id);
      }
    } catch {} finally { setLoading(false); requestAnimationFrame(() => setShowContent(true)); }
  }, [fighterId]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(API_BASE, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fighter_id: fighterId, body: body.trim(), parent_id: null }) });
  ...
  async function handleReplySubmit(parentId: string, replyBody: string) {
    try {
      const res = await fetch(API_BASE, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fighter_id: fighterId, body: replyBody, parent_id: parentId }) });
      if (res.ok) { setReplyingTo(null); setReplyMention(""); await fetchComments(); }
    } catch {}
  }

  async function handleDelete(commentId: string) {
    try { const res = await fetch(`${API_BASE}?id=${commentId}`, { method: "DELETE" }); if (res.ok) await fetchComments(); } catch {}
  }

  async function handleLike(commentId: string) {
    if (!currentUserId) return;
    setComments((prev) => prev.map((c) => c.id === commentId ? { ...c, is_liked: !c.is_liked, like_count: c.like_count + (c.is_liked ? -1 : 1) } : c));
    ...
    <div>
      ...
      {currentUserId ? (
        ...
        <div className="mt-4 border-t border-[var(--bp-line)] pt-3">
          <p className="text-center text-xs text-[var(--bp-muted)]">{t("discussion.loginRequired")}</p>
        </div>
      )}
    </div>
  );
